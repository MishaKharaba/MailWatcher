...
public class ASWBXML {
	...
	public void loadBytes(byte[] byteWBXML) throws Exception {
		...
		// Add the declaration
		// TODO: XmlDeclaration xmlDeclaration =
		// xmlDoc.createXmlDeclaration("1.0", "utf-8", null);
		// TODO: xmlDoc.insertBefore(xmlDeclaration, null);
		...
		while (bytes.getCount() > 0) {
			...
			case GlobalTokens.SWITCH_PAGE:
				...
				if (newCodePage >= 0 && newCodePage < 25) {
					...
					throw new Exception(
							String.format("Unknown code page ID 0x{0:X} encountered in WBXML", currentByte));
				}
				break;
			...
			case GlobalTokens.STR_T:
				throw new Exception(String.format("Encountered unknown global token 0x{0:X}.", currentByte));
			default:
				...
				if (hasAttributes)
					throw new Exception(String.format("Token 0x{0:X} has attributes.", token));
	...
	private byte[] encodeNode(Node node) throws Exception {
		...
		if (nodeType == Node.ELEMENT_NODE) {
			...
			if (setCodePageByXmlns(node.getPrefix())) {
				...
				// page switches.
				byteList.write((byte) GlobalTokens.SWITCH_PAGE);
			...
			if (node.hasChildNodes()) {
				...
				byteList.write((byte) GlobalTokens.END);
			}

		} else if (nodeType == Node.TEXT_NODE) {
			byteList.write((byte) GlobalTokens.STR_I);
			...
			byteList.write((byte) GlobalTokens.OPAQUE);
	...
	private boolean setCodePageByXmlns(String xmlns) throws Exception {
		...
		throw new Exception(String.format("Unknown Xmlns: {0}.", xmlns));
	}
	...
	private byte[] encodeString(String value) throws Exception {
		...
		for (int i = 0; i < charArray.length; i++) {
			byteList.write((byte) charArray[i]);
		}
		byteList.write(0x00);
		return byteList.toByteArray();
	}
